...
try
	print_string "Nom du fichier contenant la formule à dériver : ";
	print_newline();
	let inf = input_line std_in in
	let lexbuf = lexing__create_lexer_channel (open_in inf) in
	while true do
		let result = parser__Main  lexer__Token lexbuf in
			...
			print_newline();
			print_string "Selon quelle variable la dériver ? ";
			print_newline();
			...
			print_newline();
			print_string "puis simplifiée donne      : ";
